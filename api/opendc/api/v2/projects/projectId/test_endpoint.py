...
def test_update_project_missing_parameter(client):
    assert '400' in client.put(f'/api/v2/projects/{test_id}').status


def test_update_project_non_existing(client, mocker):
    mocker.patch.object(DB, 'fetch_one', return_value=None)
    assert '404' in client.put(f'/api/v2/projects/{test_id}', json={'project': {'name': 'S'}}).status
...
def test_update_project_not_authorized(client, mocker):
    ...
    assert '403' in client.put(f'/api/v2/projects/{test_id}', json={'project': {'name': 'S'}}).status
...
def test_update_project(client, mocker):
    ...
    res = client.put(f'/api/v2/projects/{test_id}', json={'project': {'name': 'S'}})
    assert '200' in res.status


def test_delete_project_non_existing(client, mocker):
    mocker.patch.object(DB, 'fetch_one', return_value=None)
    assert '404' in client.delete(f'/api/v2/projects/{test_id}').status
...
def test_delete_project_different_user(client, mocker):
    ...
    assert '403' in client.delete(f'/api/v2/projects/{test_id}').status
...
def test_delete_project(client, mocker):
    ...
    res = client.delete(f'/api/v2/projects/{test_id}')
    assert '200' in res.status
